...
#include <algorithm>
#include "ReverseStateGenStrategy.h"

namespace pea_tsp::algo::sa_helpers {

std::vector<int> ReverseStateGenStrategy::operator()(const std::vector<int> &current_state) const {
  ...
  std::ranges::reverse(next_state.begin() + n_one, next_state.begin() + n_two);
